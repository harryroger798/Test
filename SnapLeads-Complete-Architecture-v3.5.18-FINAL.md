...
**File layout (v3.5.18 — current):**
```
snapleads-downloads/
  SnapLeads-Setup-3.5.18.exe             (Windows NSIS installer, ~340MB)
  SnapLeads-3.5.18-arm64-mac.zip         (macOS zip, ~278MB)
```
...
backend/
  app/
    ...
    services/
      ...
      database_search.py        -- DuckDB S3 queries against 190M+ leads (1837+ lines, v3.5.18 SSL cert fix + autoload)
...
### v3.5.18 SSL Certificate Fix — TRUE Root Cause (Claude Opus 4.6 Analysis)

The DuckDB httpfs extension is required for S3 access to the 190M+ leads database. v3.5.12-v3.5.17 all returned 0 LinkedIn leads despite successful local testing. Claude Opus 4.6 identified the TRUE root cause:

**ROOT CAUSE: SSL/CA Certificates Missing in PyInstaller Bundle**
- DuckDB's httpfs extension uses OpenSSL directly (NOT Python's `ssl` module) to make HTTPS requests to S3
- In PyInstaller `--onefile` bundles, the compiled-in CA certificate path points to the CI build machine's location which doesn't exist on the user's machine
- Result: Every S3 HTTPS request fails with SSL certificate verification error
- The exception is caught silently, returning 0 leads
- This explains why v3.5.12-v3.5.17 all failed — they fixed httpfs LOADING but not httpfs HTTPS CONNECTIVITY

**Previous fixes (v3.5.14-v3.5.17) addressed symptoms, not root cause:**
...
- v3.5.17: DLL search path + backslash escaping + error surfacing (correct, but SSL still failed)

**v3.5.18 Solution:**
1. **`_configure_ssl_certificates()`** — New function that sets `SSL_CERT_FILE` env var from certifi CA bundle BEFORE any DuckDB connection is created
2. **4-strategy SSL cert discovery:** certifi package → bundled cacert.pem in _MEIPASS → copy to stable AppData path → Windows system certs
3. **`certifi` as explicit dependency** — Added to pyproject.toml and pip install
4. **`--collect-all certifi`** — PyInstaller collects entire certifi package including cacert.pem
5. **DuckDB autoload enabled** — `SET autoinstall_known_extensions = true; SET autoload_known_extensions = true;`
6. **Simplified httpfs loading cascade:** LOAD (bundled) → INSTALL+LOAD (network) → FORCE INSTALL+LOAD
7. **Comprehensive SSL diagnostic logging** on every connection

**Local test:** DuckDB + httpfs + S3 query returned 50,000 rows with SSL_CERT_FILE set
...
### Current URLs (v3.5.18)

| Platform | Download URL |
|----------|-------------|
| Windows | `https://f005.backblazeb2.com/file/snapleads-downloads/SnapLeads-Setup-3.5.18.exe` |
| macOS | `https://f005.backblazeb2.com/file/snapleads-downloads/SnapLeads-3.5.18-arm64-mac.zip` |

### SHA256 Hashes (v3.5.18)

| File | SHA256 |
|------|--------|
| Windows .exe | `440d837f5ae0947114d9b839df5d830e5600ca0f24178f58bc1187de5f276541` |
| macOS .zip | `d31a1048d8c9abcc8f68f3dacc12f783befdc79bd212d0033c6b381b1b6ab717` |

### VirusTotal Scan URLs (v3.5.18)

| File | VirusTotal URL |
|------|---------------|
| Windows .exe | `https://www.virustotal.com/gui/file/440d837f5ae0947114d9b839df5d830e5600ca0f24178f58bc1187de5f276541` |
| macOS .zip | `https://www.virustotal.com/gui/file/d31a1048d8c9abcc8f68f3dacc12f783befdc79bd212d0033c6b381b1b6ab717` |
