...
**File layout (v3.5.15 — current):**
```
snapleads-downloads/
  SnapLeads-Setup-3.5.15.exe             (Windows NSIS installer, ~340MB)
  SnapLeads-3.5.15-arm64-mac.zip         (macOS zip, ~278MB)
```
...
backend/
  app/
    ...
    services/
      ...
      database_search.py        -- DuckDB S3 queries against 190M+ leads (1380+ lines, v3.5.15 httpfs bundle-first fix)
...
### v3.5.15 Bundle-First Extension Strategy

The DuckDB httpfs extension is required for S3 access to the 190M+ leads database. In PyInstaller builds, the extension download fails silently because the runtime environment lacks proper network/filesystem access.

**Solution: Bundle-First Approach**
1. **CI Build Phase:** Pre-install httpfs extension via `duckdb.connect()` + `INSTALL httpfs`
2. **Zip for Codesign:** Zip extension files into `httpfs_bundle.zip` (prevents macOS PyInstaller codesign failure on `.duckdb_extension` files)
3. **PyInstaller Bundle:** Add zip to binary via `--add-data "httpfs_bundle.zip:."`
4. **Runtime Extraction:** `_ensure_bundled_httpfs()` extracts zip to writable temp dir on first run
5. **Load Extension:** `LOAD httpfs` uses the extracted extension — no network download needed
...
### Current URLs (v3.5.15)

| Platform | Download URL |
|----------|-------------|
| Windows | `https://f005.backblazeb2.com/file/snapleads-downloads/SnapLeads-Setup-3.5.15.exe` |
| macOS | `https://f005.backblazeb2.com/file/snapleads-downloads/SnapLeads-3.5.15-arm64-mac.zip` |

### SHA256 Hashes (v3.5.15)

| File | SHA256 |
|------|--------|
| Windows .exe | `64b2e873d78aebdeba36cd3e71ce591e3602cb45757e1444253330896951dca6` |
| macOS .zip | `17bbf3c482b253c378a2f882dba9a69405eb8fcd3e2c31e593c97b8782f11ad3` |

### VirusTotal Scan URLs (v3.5.15)

| File | VirusTotal URL |
|------|---------------|
| Windows .exe | `https://www.virustotal.com/gui/file/64b2e873d78aebdeba36cd3e71ce591e3602cb45757e1444253330896951dca6` |
| macOS .zip | `https://www.virustotal.com/gui/file/17bbf3c482b253c378a2f882dba9a69405eb8fcd3e2c31e593c97b8782f11ad3` |
