...
**File layout (v3.5.16 — current):**
```
snapleads-downloads/
  SnapLeads.Setup.3.5.16.exe             (Windows NSIS installer, ~340MB)
  SnapLeads-3.5.16-arm64-mac.zip         (macOS zip, ~278MB)
```
...
backend/
  app/
    ...
    services/
      ...
      database_search.py        -- DuckDB S3 queries against 190M+ leads (1380+ lines, v3.5.16 three-bug fix)
...
### v3.5.16 Three-Bug Fix (Claude Opus 4.6 Analysis)

The DuckDB httpfs extension is required for S3 access to the 190M+ leads database. v3.5.12-v3.5.15 all returned 0 LinkedIn leads despite successful local testing. Claude Opus 4.6 identified three interacting root causes:

**Root Cause #1 — Windows Backslash Escaping (75% likely):**
- `SET extension_directory='C:\Users\...'` — DuckDB SQL treats `\U`, `\A` as escape sequences
- Fix: Normalize all backslashes to forward slashes before SET statement

**Root Cause #2 — Version-Aware Extension Extraction (85% likely):**
- CI bundles httpfs under `v{CI_version}/{platform}/` but runtime DuckDB may be different version
- DuckDB looks at `v{runtime_version}/{platform}/` — version mismatch = extension not found
- Fix: Read runtime `duckdb.__version__`, extract to exact path DuckDB expects

**Root Cause #3 — Silent Exception Swallowing (95% likely):**
- All DuckDB query exceptions were caught and returned `[]`, masking real failures
- Fix: Log full error with SQL, extension state, and file listing for diagnosis

**Solution: Bundle-First + Three-Bug Fix**
1. **CI Build Phase:** Pre-install httpfs extension via `duckdb.connect()` + `INSTALL httpfs`
2. **Zip for Codesign:** Zip extension files into `httpfs_bundle.zip`
3. **PyInstaller Bundle:** Add zip to binary via `--add-data "httpfs_bundle.zip:."`
4. **Runtime Extraction (v3.5.16):** `_ensure_bundled_httpfs()` reads runtime DuckDB version, extracts to exact `v{version}/{platform}/` path
5. **Path Fix (v3.5.16):** Backslashes normalized to forward slashes before SET
6. **Error Surfacing (v3.5.16):** Full diagnostic logging on all failures
7. **Load Extension:** `LOAD httpfs` uses the version-correct extracted extension
...
### Current URLs (v3.5.16)

| Platform | Download URL |
|----------|-------------|
| Windows | `https://f005.backblazeb2.com/file/snapleads-downloads/SnapLeads.Setup.3.5.16.exe` |
| macOS | `https://f005.backblazeb2.com/file/snapleads-downloads/SnapLeads-3.5.16-arm64-mac.zip` |

### SHA256 Hashes (v3.5.16)

| File | SHA256 |
|------|--------|
| Windows .exe | `2c72f724c33d66138ad541e095875deaf3d5846950b25f3ab695d83b9c30acdd` |
| macOS .zip | `93541ee3459b4d66c4c8e5bcddac7052c2da423d0263e25b675bb778a1372911` |

### VirusTotal Scan URLs (v3.5.16)

| File | VirusTotal URL |
|------|---------------|
| Windows .exe | `https://www.virustotal.com/gui/file/2c72f724c33d66138ad541e095875deaf3d5846950b25f3ab695d83b9c30acdd` |
| macOS .zip | `https://www.virustotal.com/gui/file/93541ee3459b4d66c4c8e5bcddac7052c2da423d0263e25b675bb778a1372911` |
...
| #99 | v3.5.16: Three-bug fix for 0-leads (backslash escaping + version-aware extraction + error surfacing) | Merged |
...
| #92 | Update download links + redirects to v3.5.16 | Merged |
...
| v3.5.16 | v3.5.16 | Mar 15, 2026 | Three-bug fix: Windows backslash escaping + version-aware extraction + error surfacing (Claude Opus 4.6 analysis) |
