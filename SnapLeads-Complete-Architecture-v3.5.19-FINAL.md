...
**File layout (v3.5.19 — current):**
```
snapleads-downloads/
  SnapLeads-Setup-3.5.19.exe             (Windows NSIS installer, ~340MB)
  SnapLeads-3.5.19-arm64-mac.zip         (macOS zip, ~278MB)
```
...
backend/
  app/
    ...
    services/
      ...
      database_search.py        -- DuckDB S3 queries against 190M+ leads (1869 lines, v3.5.19 ca_cert_file fix + autoload)
...
User enters keyword + selects platform(s)
    ...
    Tier-aware: free=10 datasets, starter=15, pro=25 (v3.5.12)
...
### v3.5.19 ca_cert_file Fix — THE DEFINITIVE ROOT CAUSE (EC2-Validated)

The DuckDB httpfs extension is required for S3 access to the 190M+ leads database. v3.5.12-v3.5.18 all returned 0 LinkedIn leads despite successful local testing. Deep analysis revealed the DEFINITIVE root cause:

**ROOT CAUSE: DuckDB's httpfs uses cpp-httplib which reads CA certs from DuckDB's own `ca_cert_file` setting — NOT from SSL_CERT_FILE env var**
- DuckDB's httpfs extension uses cpp-httplib (C++ HTTP library) for HTTPS
- cpp-httplib has its own CA certificate loading that reads from DuckDB's `SET ca_cert_file` setting
- Setting `SSL_CERT_FILE` env var (v3.5.18's approach) has NO effect on cpp-httplib
- In PyInstaller bundles, the compiled-in OpenSSL cert path doesn't exist → all HTTPS fails silently → 0 leads
- DuckDB PR #10704 (Feb 2024) exposed `ca_cert_file` as a configurable setting
...
- v3.5.15: Bundle-first httpfs (correct, but SSL still failed)
- v3.5.17: DLL search path + error surfacing (correct, but SSL still failed)
- v3.5.18: SSL_CERT_FILE env var (correct concept, but DuckDB ignores env vars for this)

**v3.5.19 Solution:**
1. **`SET ca_cert_file='<certifi_path>'`** — After httpfs loads, explicitly tell DuckDB where CA certs are via its own setting (not env var)
2. **certifi package** — `certifi.where()` resolves correctly inside PyInstaller `_MEIPASS` to bundled `cacert.pem`
3. **Fallback chain:** certifi → SSL_CERT_FILE env var → skip (with warning)
4. **All v3.5.18 fixes retained:** `_configure_ssl_certificates()`, autoload, httpfs cascade, SSL diagnostic logging

**EC2 Windows Validation (FIRST EVER SUCCESSFUL WINDOWS TEST):**
- Spun up Windows EC2 instance (i-0d9da0ae62074aa71) with SSM
- Downloaded v3.5.19 exe, extracted with 7-Zip, ran backend
- DuckDB diagnostic: ALL PASS — httpfs loaded, S3 config works, test query returned 50,000 rows
- DB-Only extraction with "Software Developer" keyword: **24 leads, 23 emails returned**
- Both test sessions completed successfully (session IDs verified)
- Bundled extension has ABI mismatch (expected) but network INSTALL fallback works because ca_cert_file enables HTTPS
...
### Current URLs (v3.5.19)

| Platform | Download URL |
|----------|-------------|
| Windows | `https://f005.backblazeb2.com/file/snapleads-downloads/SnapLeads-Setup-3.5.19.exe` |
| macOS | `https://f005.backblazeb2.com/file/snapleads-downloads/SnapLeads-3.5.19-arm64-mac.zip` |

### SHA256 Hashes (v3.5.19)

| File | SHA256 |
|------|--------|
| Windows .exe | `2a629e921948f298a1bc5272015040c057b0678d8d18be394cb8d7fcc973e4b7` |
| macOS .zip | `744ae86832b963322d0645db44400ce17f251c468fa8fa6595137a9c6a8a9366` |

### VirusTotal Scan URLs (v3.5.19)

| File | VirusTotal URL |
|------|---------------|
| Windows .exe | `https://www.virustotal.com/gui/file/2a629e921948f298a1bc5272015040c057b0678d8d18be394cb8d7fcc973e4b7` |
| macOS .zip | `https://www.virustotal.com/gui/file/744ae86832b963322d0645db44400ce17f251c468fa8fa6595137a9c6a8a9366` |
