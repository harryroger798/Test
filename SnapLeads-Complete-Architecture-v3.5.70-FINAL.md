# SnapLeads Complete Architecture Document — v3.5.70

**Date:** March 23, 2026
**Version:** 3.5.70
**Status:** Production-ready (Citation Checker + GBP Detection hardened)

---

## 1. What's New in v3.5.70

### Citation Checker Engine Health + Dedicated Budget + Timeouts

**Problem:** Citation checker took **28 minutes** and returned 0 results when search engines were exhausted from prior extraction sessions. All 4 engines (Brave, DDG Lite, Bing, SearXNG) were rate-limited (429/503/202), causing 30 sequential directory checks × 4 engine retries each = 28 minutes of guaranteed-zero results.

**Root Cause:** Citation checker shared the same engine health pool as the extraction pipeline. After a user ran extractions, all engines entered cooldown. The citation checker had no mechanism to detect this — it just queued 120+ requests that all returned empty.

**5 Fixes Implemented (all additive — zero changes to extraction pipeline):**

| Fix | File | What it does |
|-----|------|-------------|
| **Engine Soft Reset** | `citation_checker.py` | `_reset_engines_for_citation_check()` — clears stale cooldowns/empty streaks before starting. Gives citation checker fresh engine access without clearing hard failure history. |
| **Engine Availability Pre-check** | `citation_checker.py` | `_check_engine_availability()` — checks if at least 1 engine has `is_available == True`. If ALL engines are in hard cooldown (consecutive_failures >= 2), returns early with "try again in a few minutes" instead of spending 28 minutes on guaranteed zeros. |
| **Per-source 15s Timeout** | `citation_checker.py` | `asyncio.wait_for(..., timeout=15)` wraps each directory check. Prevents a single slow/hung engine call from blocking the entire pipeline. |
| **Consecutive Failure Bail-out** | `citation_checker.py` | After 5 straight zero-result directory checks (`_MAX_CONSECUTIVE_EMPTY = 5`), stops early. Remaining sources marked as `engines_exhausted`. |
| **Total 5-minute Hard Timeout** | `citation_checker.py` | `time.monotonic()` tracks total elapsed. After 300s (`_TOTAL_TIMEOUT_SECS`), stops and marks remaining sources as `timeout`. |

**Expected Improvement:**
- Before: 28 minutes, 0 results when engines exhausted
- After: <1 second early return with clear "try again" message, OR <5 minutes max with results

**GBP Detection:** Already working correctly (confirmed in log analysis). "TextShift" genuinely has no Google Business Profile — the detector correctly returns "unclaimed" in 2.6 seconds. No changes needed.

### Code Changes

| File | Lines Changed | Description |
|------|--------------|-------------|
| `citation_checker.py` | +120 lines | Added `_reset_engines_for_citation_check()`, `_check_engine_availability()`, timeout constants, per-source `asyncio.wait_for()`, consecutive empty tracking, total timeout, detailed logging |
| `package.json` | version bump | `3.5.69` → `3.5.70` |

### PRs

| PR | Repo | Description |
|----|------|-------------|
| #158 | social-lead-extractor-pro | v3.5.70 — Citation checker engine health + dedicated budget + timeouts (merged) |
| #156 | snapleads-platform | Update platform download links to v3.5.70 (merged) |
...
|----------|------|--------|
| Windows | `SnapLeads Setup 3.5.70.exe` | `https://f005.backblazeb2.com/file/snapleads-downloads/SnapLeads%20Setup%203.5.70.exe` |
| macOS | `SnapLeads-3.5.70-arm64-mac.zip` | `https://f005.backblazeb2.com/file/snapleads-downloads/SnapLeads-3.5.70-arm64-mac.zip` |

**Build:** GitHub Actions — both Windows and macOS succeeded.
**Tag:** `v3.5.70` pushed to origin.
**Platform links:** PR #156 merged — download links updated on getsnapleads.store.
...
|------|---------|-----------|
| `harryroger798/social-lead-extractor-pro` | Desktop Electron app (backend + frontend) + weekly sync | **PR #158 (v3.5.70 — citation checker fixes)** |
| `harryroger798/snapleads-platform` | License server API + Marketing website (getsnapleads.store) | **PR #156 (v3.5.70 — platform links update)** |
...
- **Citation Checker** (30 directory sources, v3.5.70 hardened with engine health + timeouts)
