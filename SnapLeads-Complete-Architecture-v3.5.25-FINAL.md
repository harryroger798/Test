...
21. [Version History (v3.4.3 — v3.5.12)](#21-version-history)
...
+--------------------------+----------------------------+
         ...
         |                                    |
+--------v---------+             +-----------v----------+
| Render Search API |             |  License Server     |
| snapleads-search- |             |  snapleads-api      |
| api.onrender.com  |             |  .onrender.com      |
| ($7/mo Starter)   |             +----------+----------+
+--------+---------+                         |
         |                        +----------v----------+
+--------v---------+              |  Marketing Site     |
| iDrive E2 S3     |              |  getsnapleads.store  |
| 190M+ leads      |              |  (Netlify)          |
| (us-west-1)      |              |  _redirects -> B2   |
+------------------+              +-----------------------+
```
...
| Search API Server | FastAPI + DuckDB on Render ($7/mo Starter, US West) |
...
### Repository 3: `bytepassperks/snapleads-search-api` (Render Search API) — v3.5.25+

- **GitHub URL:** https://github.com/bytepassperks/snapleads-search-api (PRIVATE)
- **Mirrored from:** `harryroger798/snapleads-search-api` (also PRIVATE)
- **Default branch:** main
- **Language:** Python (FastAPI + DuckDB)
- **Purpose:** Server-side S3 queries for fast lead search (~2-9s vs ~100s from residential internet)
- **Deployed on:** Render.com ($7/month Starter, always-on)
- **URL:** https://snapleads-search-api.onrender.com
- **Service ID:** srv-d6ra676a2pns73adhd20
- **Endpoints:**
  - `GET /health` — Health check
  - `POST /api/search` — Hybrid search (keyword, platforms, location, tier)
...
```
GitHub PAT (harryroger798): <REDACTED — stored in Devin secrets as GITHUB_PAT>
GitHub PAT (bytepassperks): <REDACTED — stored in Devin secrets as BYTEPASSPERKS_PAT>
Owner: harryroger798
Mirror Owner: bytepassperks
```
...
**File layout (v3.5.25 — current):**
```
snapleads-downloads/
  SnapLeads-Setup-3.5.25.exe             (Windows NSIS installer, ~340MB)
  SnapLeads-3.5.25-arm64-mac.zip         (macOS zip, ~278MB)
```
...
backend/
  app/
    ...
    services/
      ...
      database_search.py        -- DuckDB S3 queries against 190M+ leads (2252 lines, v3.5.25 Render API integration + S3 fallback)
      ...
      log_service.py            -- Persistent logging service (v3.5.22): rotating file handlers, log list/tail/export/clear API
...
## 5. License Server, Search API & Web Platform
...
### Search API Server (snapleads-search-api) — v3.5.25+

- **Host:** Render.com ($7/month Starter plan, always-on, no cold starts)
- **URL:** https://snapleads-search-api.onrender.com
- **Framework:** FastAPI + DuckDB + Python 3.11.6
- **Region:** Oregon (US West) — fast access to iDrive E2 S3 (us-west-1)
- **Repo:** `bytepassperks/snapleads-search-api` (PRIVATE, mirrored from harryroger798)
- **Render Account:** noreply@mail.textshift.org
- **Purpose:** Eliminates ~100s latency for residential internet users by running DuckDB S3 queries server-side (~2-9s)
- **Endpoints:**
  - `GET /health` → `{"status": "ok", "version": "1.0.0"}`
  - `POST /api/search` → `{"leads": [...], "count": N, "query_time_ms": N}`
- **Request schema:** `{keyword, platforms, location, max_results, tier, expanded_terms}`
- **S3 credentials:** Set as environment variables in Render dashboard (not in code)
- **Flow:** Desktop app → Render API (HTTPS) → iDrive E2 S3 (DuckDB) → JSON response
...
User enters keyword + selects platform(s)
    ...
    (S3 has zero B2B data). Mixed selections run DB search normally.
          |
    [database_search.py]
    v3.5.25: Try Render API first (~2-9s server-side queries)
    Falls back to direct S3 DuckDB if API unreachable (~100s)
    Tier-aware: free=3, starter=5, pro=5, unlimited=8
    120s timeout per query, 600s master timeout, semaphore(3) concurrency
          |
...
### Current URLs (v3.5.20)

| Platform | Download URL |
|----------|-------------|
| Windows | `https://f005.backblazeb2.com/file/snapleads-downloads/SnapLeads-Setup-3.5.20.exe` |
| macOS | `https://f005.backblazeb2.com/file/snapleads-downloads/SnapLeads-3.5.20-arm64-mac.zip` |

### SHA256 Hashes (v3.5.20)

| File | SHA256 |
|------|--------|
| Windows .exe | `aa07abe04144cf9332c2129975aca1d9fccbcbf1f34c1aaecca062e8a4eaed71` |
| macOS .zip | *(pending VT scan)* |

### VirusTotal Scan URLs (v3.5.20)

| File | VirusTotal URL |
|------|---------------|
| Windows .exe | `https://www.virustotal.com/gui/file/aa07abe04144cf9332c2129975aca1d9fccbcbf1f34c1aaecca062e8a4eaed71` |
| macOS .zip | *(pending VT scan)* |
...
- **MAJOR: Render API Integration — server-side S3 queries (~9s vs ~100s)**
- New FastAPI search API server deployed on Render ($7/month Starter, always-on)
- Desktop app calls Render API first (fast ~2-9s), falls back to direct S3 if API unreachable
- API server runs DuckDB queries against iDrive E2 S3 from US West region (low latency)
- New function `_search_via_render_api()` in `database_search.py` — HTTP POST to Render API
- `search_database_hybrid()` modified: try API → on failure, fall back to direct S3 DuckDB
- Master timeout raised from 180s to 600s (3x safety margin for fallback path)
- Country semaphore raised from 2→3 for faster parallel queries in fallback mode
- Partial results preservation: shared list collects results even if master timeout fires
