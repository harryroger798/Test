...
21. [Version History (v3.4.3 — v3.5.30)](#21-version-history)
...
**File layout (v3.5.30 — current):**
```
snapleads-downloads/
  v3.5.30/
    SnapLeads-Setup-3.5.30.exe           (Windows NSIS installer, ~690MB — includes Patchright Chromium + driver)
    SnapLeads-3.5.30-arm64-mac.zip       (macOS zip, ~292MB)
  v3.5.29/
    SnapLeads-Setup-3.5.29.exe           (Windows NSIS installer, ~688MB)
...
backend/
  app/
    ...
    services/
      ...
      anti_detection.py         -- curl_cffi TLS fingerprint with auto-probe fallback chain (chrome131→124→120→116), rate limiting, SSRF protection (580+ lines, v3.5.30)
      quality_scorer.py         -- Lead quality scoring 0-100 (205 lines)

      ## Live Scraping (v3.5.0+) ##
      live_scrapers.py          -- 12-platform live scrapers (2800+ lines, v3.5.30: Facebook 14-source pipeline):
                                   Reddit, WhatsApp, Facebook (v3.5.30: 14-source pipeline with 5 Claude-recommended fixes), Telegram, Twitter,
      ...
      ## Google Dorking ##
      google_dorking.py         -- Multi-engine dorking with 10+ templates per platform + DuckDuckGo via Patchright (v3.5.30: cookie priming + .results--main wait + uddg URL unwrap + DDG Lite fallback) + HTTP fallback (v3.5.26: httpx+BS4 when Patchright unavailable)
...
### v3.5.30 (March 16, 2026)
- **5 Claude-recommended fixes for Facebook pipeline** — expands from 10-source to 14-source pipeline
- **Fix 1 — `anti_detection.py`:** Removes non-existent `chrome130` impersonation. Adds `_best_impersonation()` probe function with fallback chain (`chrome131→124→120→116`) and caches the result at module level.
- **Fix 2 — `google_dorking.py`:** Rewrites DDG Patchright search to fix 3 compounding bugs: missing cookie priming (visit duckduckgo.com first), wrong wait condition (`domcontentloaded` → `.results--main` selector), and DDG redirect URL unwrapping (`uddg=` parameter). Adds DDG Lite fallback + Google HTTP fallback.
- **Fix 3 — `live_scrapers.py`:** Adds Sulekha XHR endpoint (`/localserv/splistings`) with HTML fallback. Rewrites Yellow Pages India scraper for `yellowpages.in` with Cloudflare bypass via chrome131 impersonation and pagination.
- **Fix 4 — `live_scrapers.py`:** Replaces dead Google Cache enrichment (removed March 2024) with 6-strategy pipeline: Bing Cache, Wayback CDX, archive.ph, Google name search, Bing name search, email pattern construction.
- **Fix 5 — `live_scrapers.py`:** Adds 4 new high-yield sources: WedMeGood JSON API (wedding-only gate), ShaadiSaga HTML (wedding-only gate), JustDial XHR paginated (replaces 1-result version), Google Maps JSON-LD structured data extraction.
- **Bug fix:** WedMeGood/ShaadiSaga gated on wedding intent only (not all Indian queries) — fixes CodeRabbit-caught logic error.
- PRs: [#117](https://github.com/harryroger798/social-lead-extractor-pro/pull/117)
...
*End of Consolidated Architecture Document — v3.5.30*
