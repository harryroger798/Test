# SnapLeads Complete Architecture Document -- v3.5.55

**Version:** 3.5.55 (Data Quality Fixes -- Email Sanitization + Country TLD Inference)
**Release Date:** March 19, 2026
**Previous Version:** 3.5.54
**Status:** CODE CHANGES -- 2 additive data quality fixes. Zero pipeline logic changes.
...
v3.5.55 implements **2 additive data quality fixes** identified during the v3.5.54 lead quality verification:

1. **Fix 1: Email Sanitization** (`_sanitize_email()` in `database_search.py`) -- Strips leaked "Email: " prefix and splits concatenated emails. Affects only 0.66% of emails (21/3,183). 99.3% of clean emails pass through unchanged.
2. **Fix 2: Unknown Country TLD Inference** (Signal 5 in `database_search.py` + Signal 6 in `routes.py`) -- Recovers country data for ~30% of LinkedIn leads that had "Unknown" country by checking source_url and website domain TLDs (e.g., `.au` → Australia, `.co.uk` → United Kingdom).

These fixes are purely additive -- they sanitize output data without modifying any pipeline logic. The v3.5.53 pipeline remains fully verified and unchanged.

### v3.5.54 Key Finding: ALL 8 TEST SESSIONS COMPLETED SUCCESSFULLY (Unchanged in v3.5.55)
...
## Architecture Overview (Current -- v3.5.55)
...
### Stable Period (v3.5.44 -> v3.5.55)

After v3.5.44 stabilized Group A+B, subsequent versions (v3.5.45-v3.5.55) were targeted fixes that didn't regress previous groups:
...
### Fix 1: Email Sanitization (`_sanitize_email()` in `database_search.py`)

**Problem:** 21 out of 3,183 emails (0.66%) had data quality issues from PAN India CSV data:
- "Email: " prefix leaked into field (e.g., `"Email: jrmehta@bom2.vsnl.net.in"`)
- Two emails concatenated in one field (e.g., `"gayatrimanoj@hotmail.com gayatrimanoj@yahoo.com"`)

**Fix:** New `_sanitize_email()` function (lines 1891-1913 of `database_search.py`):
```python
...
**Applied in:** `_linkedin_row_to_lead()`, `_instagram_row_to_lead()`, `_pan_india_row_to_lead()`

**Impact:** 99.3% of clean emails pass through unchanged. Only the 0.66% malformed ones get cleaned.

### Fix 2: Unknown Country TLD Inference

**Problem:** 438 out of 1,433 Restaurant leads (30.5%) from the no-location global search (T19) had "Unknown" country. These were LinkedIn leads without location data in any existing signal.

**Fix A — Signal 5 in `_infer_lead_country()` (`database_search.py`, lines 150-159):**
```python
# Signal 5 (v3.5.55): Infer from source_url domain TLD
...
**Fix B — Signal 6 in `_infer_country_from_lead()` (`routes.py`, lines 243-254):**
```python
# Signal 6 (v3.5.55): Infer from website/source_url TLD
for url_field in ("website", "source_url"):
    url_val = lead_data.get(url_field, "").strip().lower()
    if url_val:
        for tld, country in sorted(_EMAIL_TLD_TO_COUNTRY.items(), key=lambda x: -len(x[0])):
            tld_suffix = tld.lstrip(".")
            if f".{tld_suffix}/" in url_val or url_val.endswith(f".{tld_suffix}"):
                return country
```

**Impact:** Recovers ~30% of "Unknown" country leads by mapping domain TLDs to countries (e.g., `.au` → Australia, `.co.uk` → United Kingdom, `.de` → Germany). Only activates when all 4 previous signals (phone prefix, email TLD, location field, keyword hint) failed to identify a country.

---

## v3.5.55 Country Inference Signal Cascade (Complete)

```
Lead arrives with country=""
    |
    v
Signal 0: Explicit country field from source → if set, return it
    |
    v
Signal 1: Phone prefix (+91 → India, +1 → US, etc.) → only trust + prefix
    |
    v
Signal 2: Email TLD (.co.in → India, .co.uk → UK, etc.)
    |
    v
Signal 3: Location/address field → city name match
    |
    v
Signal 4: Bio/industry/company text → city/country name match
    |
    v
Signal 5 (v3.5.55 NEW): source_url domain TLD (.au/ → Australia)
    |
    v
Signal 6 (v3.5.55 NEW): website/source_url domain TLD (routes.py)
    |
    v
Return "Unknown" (only if ALL 6 signals fail)
```
...
## Build Artifacts (Current -- v3.5.55)

| Platform | Filename | Size | B2 URL |
|----------|----------|------|--------|
| Windows | SnapLeads Setup 3.5.55.exe | ~657 MB | https://f005.backblazeb2.com/file/snapleads-downloads/SnapLeads%20Setup%203.5.55.exe |
| macOS | SnapLeads-3.5.55-arm64-mac.zip | ~278 MB | https://f005.backblazeb2.com/file/snapleads-downloads/SnapLeads-3.5.55-arm64-mac.zip |
...
| File | Purpose | Lines |
|------|---------|-------|
| backend/app/api/routes.py | Pipeline orchestrator + Signal 6 country inference | ~4,150 |
| backend/app/services/database_search.py | S3/DuckDB queries + email sanitization + Signal 5 | ~3,013 |
