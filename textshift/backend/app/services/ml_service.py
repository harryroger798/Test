...
class MLModelService:
    ...
    def _humanize_single(self, sentence: str, use_post_processor: bool = True, passes: int = 2, mode: str = 'casual') -> str:
        """Humanize a single sentence using SageMaker (primary) or local T5 model (fallback)."""
    ...
    def _get_roberta_prediction(self, text: str) -> Dict[str, float]:
        """Get AI probability from SageMaker (primary) or local RoBERTa model (fallback).
        
        Returns:
            Dict with 'ai_prob' and 'human_prob'
        """
        sm_result = sagemaker_client.invoke_classification("detector", text, top_k=2)
        if sm_result:
            human_prob = 0.5
            ai_prob = 0.5
            for item in sm_result:
                if item.get("label") == "LABEL_0":
                    human_prob = item["score"]
                elif item.get("label") == "LABEL_1":
                    ai_prob = item["score"]
            logger.info(f"SageMaker detector: AI={ai_prob*100:.1f}%")
            return {'ai_prob': ai_prob, 'human_prob': human_prob}

        logger.info("SageMaker detector unavailable, falling back to local RoBERTa")
        self._load_detector()
