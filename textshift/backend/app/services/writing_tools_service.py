...
class WritingToolsService:
    ...
    def _generate_with_t5(self, prompt: str, max_length: int = 256, min_length: int = 10) -> Optional[str]:
        """Generate text using SageMaker flan-t5-base (primary) or local model (fallback)."""
        try:
            ...
            if sm_result is not None:
                logger.info("SageMaker flan-t5 returned empty/invalid, falling back to local")
        except Exception as e:
    ...
    def detect_tone(self, text: str) -> Dict[str, Any]:
        """Detect emotional tone using SageMaker endpoint (primary) or local RoBERTa model (fallback)."""
        try:
            sm_results = sagemaker_client.invoke_classification("tone-detector", text, top_k=len(self.TONE_LABELS))
            if sm_results:
                tones = []
            ...
            logger.info("SageMaker tone-detector unavailable, falling back to local model")
        except Exception as e:
            logger.warning(f"SageMaker tone detection failed, falling back to local: {e}")
    ...
    def translate(self, text: str, source_lang: str, target_lang: str) -> Dict[str, Any]:
        """Translate text using SageMaker endpoint (primary) or local Helsinki-NLP models (fallback)."""
        try:
